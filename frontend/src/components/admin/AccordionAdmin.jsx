  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAgentList(agentsResponse.data);
      } catch (error) {
      ...
      setComplaintList((prevComplaints) =>
        prevComplaints.map((complaint) =>
          complaint._id === complaintId
            ? { ...complaint, status: 'assigned', agentName }
            : complaint
        )
      );

      alert(`Complaint successfully assigned to agent ${agentName}`);
    } catch (error) {
      ...
      <Accordion className="accordian" alwaysOpen>
        ...
        <Accordion.Item eventKey="0">
          ...
          <Accordion.Body style={{ background: 'aliceblue' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', margin: '20px' }}>
              {loading ? (
                <Alert variant="info">Loading complaints...</Alert>
                    ...
                    <Card.Body style={{ textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: '14px', marginTop: '20px' }}>
                        ...
                        {complaint.status === 'assigned' && (
                          <Card.Text>
                            Assigned to: <strong>{complaint.agentName}</strong>
                          </Card.Text>
                        )}
                      </div>
                      {complaint.status === 'completed' || complaint.status === 'assigned' ? null : (
        ...
        <Accordion.Item eventKey="1">
          ...
          <Accordion.Body style={{ background: 'aliceblue' }}>
            <div style={{ display: 'flex', flexWrap: 'wrap', margin: '20px' }}>
              {loading ? (
                <Alert variant="info">Loading agents...</Alert>
              ) : agentList.length > 0 ? (
                agentList.map((agent, index) => (
